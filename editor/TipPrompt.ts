		...
		switch (type) {
			...
			case "instrumentIndex": {
				message = div(
					...
					p("In the \"Channel Settings\" option from AbyssBox's \"Edit\" menu, there are a few ways to enable multiple instruments per channel."),
			...
			case "instrumentType": {
				message = div(
					h2("Instrument Type"),
					p("AbyssBox comes with many instrument presets, try them out! You can also create your own custom instruments!"),
			...
			case "chipWave": {
				message = div(
					h2("Chip Wave"),
					p("AbyssBox comes with some sound waves based on classic electronic sound chips, as well as several unique waves. This is the basic source of the sound of the instrument, which is modified by the other instrument settings."),
				);
			} break;
			case "chipNoise": {
				message = div(
					h2("Noise"),
					p("AbyssBox comes with several basic noise sounds. These do not have any distinct musical pitch, and can be used like drums to create beats and emphasize your song's rhythm."),
				);
			} break;
			...
			case "chords": {
				message = div(
					h2("Chords"),
					p("When multiple different notes occur at the same time, this is called a chord. Chords can be created in AbyssBox's pattern editor by adding notes above or below another note."),
					...
					p("Some AbyssBox instruments have an option called \"custom interval\" which uses the chord notes to control the interval between the waves of a single tone. This can create strange sound effects when combined with FM modulators."),
				);
			} break;
			case "vibrato": {
				message = div(
			...
			case "effects": {
				message = div(
					h2("Effects"),
					p("AbyssBox has many different kinds of special effects you can add to instruments. You can turn on multiple effects at once, and they can be configured individually. Try them all out!"),
				);
			} break;
			...
			case "aliases": {
				message = div(
					h2("Aliasing"),
					p("AbyssBox applies a technique called 'anti-aliasing' to instruments normally to help them sound cleaner even at high frequencies and low sample rates."),
